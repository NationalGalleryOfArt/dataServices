...
public class TMSFetcher {
    ...
    private final ArtDataManagerService managerSrv;
    private final ArtDataQuerier manager;
    
    public TMSFetcher(final DataSourceService ps, final OperatingMode mode, final ArtDataManagerService mgr)
    {
        poolService = ps;
        isPrivateConfigured = mode == OperatingMode.PRIVATE;
        managerSrv = mgr;
        manager = mgr.getArtDataQuerier();
    }
    ...
    public synchronized ArtData load() throws Exception {
        ...
        try {
            ...
            if (getIsPrivateConfigured())
            {
                ...
                newData.departments = (new DepartmentFactory(manager)).buildDepartments(getPoolService());
            }
            else
            {
            ...
            if (isPrivateConfigured)
            {
                ...
                TMSExhibitionFactory srv = new ExhibitionFactoryImpl(manager);
                if (srv != null)
                {
 ...
 // load all constituent data
    ...
    synchronized protected Map<Long, ArtObject> getArtObjects(
        ...
        ArtObject.setFetchAllObjectsQuery(managerSrv.getOperatingMode());
    ...
    synchronized protected <T extends Derivative> void loadImagery(Map<Long, ArtObject> newArtObjects, T seed) throws SQLException {
        ...
        log.info(String.format("Assinable Image Map size: %d", imgByObject.size()));
    ...
    public class TMSData
    {
        ...
        protected ArtData getArtData()
        {
        	ArtData rdata = new ArtData();
        	rdata.setArtObjects(artObjects.values());
        	rdata.setConstituents(constituents.values());
        	rdata.setDepartments(departments.values());
        	rdata.setExhibitions(exhibitions.values());
        	rdata.setLocations(locations.values());
        	rdata.setPlaces(newPlaces.values());
        	rdata.setMedia(newMediaItems.values());
        	rdata.setMediaRelationships(newMediaRelationshps);
        	rdata.setTMSLocations(newPlacesTMSLocations);
        	return rdata;
        }
        public Map<Long, ArtObject> getArtObjectData()
        {
            return artObjects;
        }
        
        public Map<Long, Constituent> getConstituentData()
        {
            return constituents;
        }
        
        public Map<Long, Location> getLocationData()
        {
            return locations;
        }
        
        public Map<Long, Exhibition> getExhbitionData()
        {
            return exhibitions;
        }
