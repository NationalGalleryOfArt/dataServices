...
public class ArtObject extends ArtEntityImpl {
	...
	protected boolean imageOK(Derivative d) {
		// we can only show an image if the object is a public object
		...
		return    isPublic() &&                                     // public object
				!isThumbnailProhibited() &&                        // thumbnails not prohibited
				( !d.isZoom() || isZoomImageryPermitted() ) &&      // if zoom image, ensure we can show zooms for this object
	...
	private Boolean canShowImagery() {
		return TypeUtils.longToBoolean(getCanShowImagery());
	}
	...
	public Boolean isThumbnailProhibited() {
		return TypeUtils.longToBoolean(getThumbnailsProhibited());
	}
	...
	// PRIVATE FIELDS NOT USED ON THE PUBLIC SITE, SOME DETAILED SOME NOT
	protected Boolean hasImagery() {
		return canShowImagery() && hasThumbnail();
	}

	protected Boolean hasThumbnail() {
		Long mde = getMaxDerivativeExtent();
		return !isThumbnailProhibited() && ( mde == null || mde <= 0 ) ? false : true;
	}

	private Long getIsPublic() {
		// always public since we're using the TMS Public Extract
	...
	public Boolean isZoomImageryPermitted() {
		return TypeUtils.longToBoolean(getZoomPermissionGranted());
	}
